...
- Every application registers with Eureka: `AUTH-SERVICE`, `USER-SERVICE`, `NOTIFICATION-SERVICE`, `AUDIT-SERVICE`, and
  `API-GATEWAY`
...
For local learning, `/internal/**` is permitted in auth-service. In a production version, it must be protected using
service-to-service authentication (mTLS, OAuth2 client credentials, or a signed internal token).
